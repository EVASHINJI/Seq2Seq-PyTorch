...
parser.add_argument('--load_checkpoint', action='store', dest='load_checkpoint',
                    help='File path of the checkpoint to load.')

parser.add_argument('--train_path', action='store', dest='train_path', help='Train data file path.')
parser.add_argument('--dev_path', action='store', dest='dev_path', help='Dev data file path.')
parser.add_argument('--test_path', action='store', dest='test_path', help='Train data file path.')
...
parser.add_argument('--resume', action='store_true', dest='resume', default=False,
                    help='Indicates if training has to be resumed from the latest checkpoint. \
                          If load_checkpoint is set, then train from loaded.')
parser.add_argument('--max_steps', action='store', dest='max_steps', 
                    help='Maximum num of steps for training.', default=500000, type=int)
parser.add_argument('--max_epochs', action='store', dest='max_epochs', 
                    help='Maximum num of epochs for training.', default=15, type=int)
parser.add_argument('--skip_steps', action='store', dest='skip_steps', 
                    help='Num of steps skipped at the beginning of training.', default=0, type=int)
parser.add_argument('--checkpoint_every', action='store', dest='checkpoint_every', 
                    help='Num of batches to checkpoint.', default=500, type=int)
parser.add_argument('--print_every', action='store', dest='print_every', 
                    help='Num of batches to print.', default=50, type=int)
...
parser.add_argument('--best_ppl', dest='best_ppl', 
                    help='Initial ppl threshold for saving best model.', default=100000.0, type=float)
...
parser.add_argument('--rnn_cell', action='store', dest='rnn_cell', 
                    help='Type of RNN cell. gru or lstm.', default='gru')
parser.add_argument('--n_hidden_layer', action='store', dest='n_hidden_layer', 
                    help='Num of hidden layer in each RNN.', default=1, type=int)
...
parser.add_argument('--beam_width', action='store', dest='beam_width', 
                    help='Beam width when using beam search decoder in inference.', default=1, type=int)
